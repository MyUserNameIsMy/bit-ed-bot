...
export class AskQuestionScene {
  ...
  async confused(@Ctx() ctx: SceneContext & ISession) {
    ...
    await ctx.reply(
      'Спасибо за ваш вопрос! 🙏 Мы переслали его нашему куратору для более детального рассмотрения. На данный момент эта функция разрабатывается. 🚀 🕵️‍♂️ В ближайшее время вы получите ответ. 📬 Если у вас будут другие вопросы, не стесняйтесь задавать их. 🤔\n',
      {
        reply_markup: await this.botService.showKeyboardMenuButtons(),
      },
    );
